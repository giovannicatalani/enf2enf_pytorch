...
from utils.dataset import SDFDataset, AirfransFlowDataset, subsample_dataset
...
def main(cfg: DictConfig) -> None:
    ...
    wandb.init(project='airfrans_enf_torch', config=OmegaConf.to_container(cfg, resolve=True))
    wandb.config.update({
    ...
    train_latents = torch.load(cfg.dataset.train_latents_path)
    test_latents = torch.load(cfg.dataset.test_latents_path)
    # Create dataset instance
    train_dataset = AirfransFlowDataset(
        train_data,
        train_names,
        geom_latents = train_latents['latents'].detach().cpu(),
        mode         = 'train',
        target_field= cfg.dataset.target_field,
    )
    coef_norm = train_dataset.coef_norm

    val_dataset = AirfransFlowDataset(
        test_data,
        test_names,
        geom_latents = test_latents['latents'].detach().cpu(),
        mode         = 'test',
        coef_norm    = coef_norm,
        target_field= cfg.dataset.target_field,
    )
    ...
    for step in tqdm(range(start_epoch, epochs)):
        ...
        if use_pred_loss:
            ...
            wandb.log({"val_loss": test_loss_in, "epoch": step})
        ...
        if test_loss_in < best_loss:
            ...
            if cfg.logging.save_predictions:
                predictions_data = {
                    ...
                    'test_names': test_names,  # Include sample names for reference
                }
