...
def train(cfg: DictConfig):
    ...
    # 4) Kick off training
    best_state, trained_enf, best_alpha_c = train_encoder(cfg, train_ds, val_ds, run_folder)
    ...
    save_all_modulations(trained_enf, best_alpha_c, cfg, train_ds, torch.device(cfg.train.device), train_mod_path)

    val_mod_path = os.path.join(mods_folder, "val_modulations.pt")
    save_all_modulations(trained_enf, best_alpha_c, cfg, val_ds, torch.device(cfg.train.device), val_mod_path)
